...
with tf.name_scope("DNN_net"):
    ...
    x_hat_dnn = (B_dnn_T @ tf.linalg.inv(B_dnn @ B_dnn_T + sigma2n * W_dnn_T @ W_dnn) @ y_bar_hat_dnn[..., None])[
        ..., 0]
#####################################################################################
######## Loss Function
...
###########  Training
path_model = './params/params_b_m_n_bits_dim_k' + str((b, m, n, bits_dim, k)) + 'n_epcohs' + str(0)
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 0:
            ...
            if epoch<=100:
                path_model = './params/params_b_m_n_bits_dim_k' + str((b, m, n, bits_dim, k)) + 'n_epcohs' + str(epoch)
    ...
    for epochs in range(500):
        if epochs % 10 == 0 and epochs <=100:
            ...
            path_model = './params/params_b_m_n_bits_dim_k' + str((b, m, n, bits_dim, k)) + 'n_epcohs' + str(epochs)
            saver.restore(sess, path_model)
            loss_val = sess.run(loss, feed_dict=feed_dict_val)
            ...
            loss_val = sess.run(loss, feed_dict=feed_dict_val)
            loss_list.append(loss_val)
    sio.savemat('BNQ.mat', {'loss_list': loss_list, 'epoch_list': epoch_list})
