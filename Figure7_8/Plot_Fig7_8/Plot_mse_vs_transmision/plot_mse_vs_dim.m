...
% load Data_performances_3_5.mat
load Data_performances5.mat

mse_lower_bound = mse_LB_k;
mse_cd = mse_CD_Q_simul;
mse_cd0 = mse_CD_simul;

load Data_performances_EVD.mat
mse_evd = mse_EVD_Q_simul;
mse_evd0 = mse_EVD_simul;
...
DNN_progressive_loss_sum = [0.020024577,0.013493723,  0.0093389405, 0.00774913, 0.0063593932];
...
% lb_idx = linspace(transmission_num_local(1),transmission_num_global(end),5);
% semilogy(lb_idx,mse_lower_bound,'o-','color',color1,'lineWidth',2,'markersize',8);
% hold on 
% semilogy(transmission_num_local,mse_evd,'v-','color',color2,'lineWidth',2,'markersize',8);
% hold on
% % semilogy(dim,mse_evd0,'v--','color',color2,'lineWidth',2,'markersize',8);
% % hold on
% semilogy(transmission_num_global,mse_cd,'>-','color',color3,'lineWidth',2,'markersize',8);
% hold on
% % semilogy(dim,mse_cd0,'>--','color',color3,'lineWidth',2,'markersize',8);
% % hold on
% semilogy(transmission_num_local,DNN,'d-','color',color4,'lineWidth',2,'markersize',8);
% hold on
% % semilogy(dim,DNN_progressive,'s-','color',color5,'lineWidth',2,'markersize',8);
% hold on
% semilogy(transmission_num_local,DNN_progressive_loss_sum,'s-','color',color5,'lineWidth',2,'markersize',8);
% hold on
% lg=legend('Lower bound', 'EVD',...
%     'BCD',...
%     'DNN ',...
%     'DNN progressive (loss=sum)',...
%     'Location','northeast');
% 
% % lg=legend('Lower bound', 'EVD','EVD without Q',...
% %     'BCD','BCD without Q',...
% %     'DNN ',...
% %     'DNN progressive (loss=sum)',...
% %     'Location','northeast');
...
% set(lg,'Interpreter','latex');
% grid on
% xlabel('Number of Dimensions $K$','Interpreter','latex','FontSize',fs2);
% ylabel('Average MSE','Interpreter','latex','FontSize',fs2);
