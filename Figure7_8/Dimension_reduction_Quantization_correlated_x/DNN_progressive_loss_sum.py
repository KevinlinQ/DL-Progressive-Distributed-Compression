...
path_model = './params/params_progressive_sum_loss_b_m_n_bits_dim_k' + str((b, m, n, bits_dim))
path_results = 'data_progressive_sum_loss_b_m_n_bits_dim_k' + str((b, m, n, bits_dim)) + '.mat'
...
with tf.name_scope("DNN_net"):
    ...
    for cnt in range(0, len(k_vec)):
        ...
        part2 = tf.concat(
            [tf.zeros([tf.shape(W_dnn1_temp)[0], m, k]), W_dnn2_temp, tf.zeros([tf.shape(W_dnn1_temp)[0], m, k])],
            axis=2)
        ...
        x_hat_dnn[cnt] = \
            (cov_matrix@B_dnn_T @ tf.linalg.inv(B_dnn@cov_matrix@B_dnn_T + sigma2n * W_dnn_T @ W_dnn) @ y_bar_hat_dnn[..., None])[..., 0]
        loss_dim[cnt] = tf.keras.losses.MSE(tf.reshape(x_dnn, [-1, 1])[:, 0], tf.reshape(x_hat_dnn[cnt], [-1, 1])[:, 0])
        loss_dim_norm.append(loss_dim[cnt])
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        print('epoch', epoch, 'lr:%0.3e' % sess.run(learning_rate), '  loss_test:%2.5f' % loss_val,
              '  best_test:%2.5f' % best_loss,
