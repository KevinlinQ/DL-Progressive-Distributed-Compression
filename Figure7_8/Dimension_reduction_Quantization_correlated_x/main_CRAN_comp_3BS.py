...
path_model = './params/params_b_m_n_bits_dim_k' + str((b, m, n, bits_dim, k))
path_results = 'data_b_m_n_bits_dim_k' + str((b, m, n, bits_dim, k)) + '.mat'
#####################################################
#####################################################
...
clip_range = tf.get_variable("clip_range", dtype=tf.float32, initializer=4 * tf.ones([1, k]))
...
with tf.name_scope("DNN_net"):
    ...
    x_hat_dnn = (cov_matrix @ B_dnn_T @ tf.linalg.inv(B_dnn @ cov_matrix @ B_dnn_T + sigma2n * W_dnn_T @ W_dnn) @ y_bar_hat_dnn[..., None])[..., 0]
#####################################################################################
######## Loss Function
...
##########################################################################
############  Validation Set 
x_dnn_val = np.random.normal(loc=0.0, scale=1.0, size=[val_size, n])@cov_matrix_sqrt
...
feed_dict_val = {x_dnn: x_dnn_val, \
                 ...
                 A3_dnn: A3_dnn_val}
############  Test Set 
x_dnn_test = np.random.normal(loc=0.0, scale=1.0, size=[test_size, n])@cov_matrix_sqrt
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        for rnd_indices in range(batch_per_epoch):
            x_dnn_batch = np.random.normal(loc=0.0, scale=1.0, size=[batch_size, n])@cov_matrix_sqrt
    ...
    loss_dnn_test, W_dnn_test, y_bar_dnn_test = sess.run([loss, W_dnn, y_bar_dnn], feed_dict=feed_dict_test)

mse_baseline_exp_test, mse_dnn_exp_test, mse_UB_exp_test = func_baseline_EVD(A1_dnn_test,A2_dnn_test,A3_dnn_test,k,n,sigma2n,W_dnn_test)

print('mse_baseline_exp_test:%2.5f'%mse_baseline_exp_test,'  mse_dnn_exp_test:%2.5f'%mse_dnn_exp_test,'  mse_UB_exp_test:%2.5f'%mse_UB_exp_test)
sio.savemat(path_results,dict(n=n,k=k,m=m,b=b,sigma2n=sigma2n,
                           test_size= test_size,
                           mse_baseline_exp_test= mse_baseline_exp_test,
                           mse_dnn_exp_test = mse_dnn_exp_test,
                           loss_dnn_test = loss_dnn_test,
                           mse_UB_exp_test = mse_UB_exp_test))
